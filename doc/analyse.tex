...
\begin{large}
	...
	\bf Technical Report :  NR-2011-01
	\end{center}
...
\noindent In this document, we choose $d=1$ and $a>0$ for simplicity. The concepts, presented afterthen, can be generalized to $d>1$, but this introduces new difficulties like anisotropy, which will be mentionned elsewhere. Besides, the discretization of $\Omega$, noted $\mathcal{T}_h$, is made using regular elements, which leads to the mesh size $h = cst$. We note $U^n_j$ the solution of the discretization of Eq.~(\ref{eq:conv_lin}), at the discrete time $t^n$ and associated with node (or control volume) located at $x_j$. It approximates $u(x_j,t^n)$.The LW differencing scheme is written\cite{Leveque:2002}:
\begin{equation}
\label{eq:LW}
...
\noindent where $k$ the time step. The LW scheme is second-order accurate both in space and time, and is $L^2 \ \mathrm{stable}$ provided $c \leq 1$ \cite{Hirsch:1988}. In the present document, we write: $\i = \sqrt{-1}$.
%--------------------------------------------------------------------------------------------------
...
%==============================
Equation (\ref{eq:conv_lin}) admits non-trivial plane wave solutions of the form:
\begin{equation}
\label{eq:plane_wave}
U=\exp(\i \xi x - \i \omega t),
\end{equation}
...
\noindent is respected \cite{Lighthill:1978,Trefethen:1996}. Identically, a discrete approximation of Eq.~(\ref{eq:conv_lin}) admits plane wave solutions (\ref{eq:plane_wave}). Therefore, modified dispersion relations can be expressed. To do so, Eq.~(\ref{eq:plane_wave}) is injected in the difference schemes to write an expression, which links $\xi$ and $\omega$. 

\vskip8mm

\begin{example}
The dispersion relation of the LW scheme is:
\begin{equation}
\label{eq:dispersion_relation_LW}
...
Let suppose that $U^n$ is a function with a finite norm, so that it can be expressed using Fourier transforms:
...
\noindent and $g(\xi) = \widehat{Z} = \widehat{U}^{n+1}(\xi) / \widehat{U}^n(\xi)$ is the \emph{amplification factor} for a given wavenumber $\xi$. On the other hand, the space Fourier transform of Eq.~(\ref{eq:conv_lin}) is:
\begin{equation}
...
$$ \begin{array}{rcl}
   ...
   \xi &\longmapsto& S^{\star}(\xi) = \Sigma^{\star}(\xi) - \i \Omega^{\star}(\xi) = \sigma^{\star} - \i \omega^{\star} ,    
   \end{array}
$$
\noindent which is the \emph{modified dispersion relation} of the difference scheme. $\omega^{\star}$ is to be compared with the real pulsation and holds the \emph{phase errors}, while $\sigma^{\star}$ is linked with the \emph{numerical diffusion} of the scheme. We can also define the modified wavenumber \cite{Hirsch:1988}:
\begin{equation}
\label{eq:kstar}
...
\noindent Furthermore, a cut-off wavenumber can be defined as the maximum wavenumber, which can be represented by the discretization\footnote{The phase error at $\xi=\xi_c$ is often big (up to 50\%). Another definition must be written to better evaluate phase accuracy. See \cite{Bailly:2004}.} and is often far smaller than $\pi / h$:
...
\end{equation}
\noindent Figure~(\ref{fig:kdx_lw}) shows the modified wavenumber of the LW scheme for different CFL numbers. The LW scheme has a good phase accuracy for larger wavelengths, but shows a poor behaviour for badly resolved scales, especially for small CFL numbers. It is noteworthy that the LW scheme has no phase error\footnote{No error at all in fact !} for $c=1$. Table~(\ref{tab:kdx_lw}) gathers the cut-off wavenumbers (and the corresponding wavelengths $\lambda=2\pi /\xi$) of the LW scheme, depending on the CFL number. Moreover, wavenumbers $k_a h$, such that:
...
\noindent Table~(\ref{tab:kdx_lw}) confirms that phase errors are lower when $c\rightarrow 1$. It also shows that $\xi^{\star}$ is relatively small, compared with $\pi$ (the supposedly maximum non-dimensionalized wavenumber), which is linked with the strong discrepencies of the scheme with small wavelengths. Phase errors are bigger than $1\%$, as soon as the wavelength is smaller than around $20 h$, for a large band of CFL numbers ($c<0.5$). This shows the dispersion errors are non-negligible, even when resolution seems rather correct (15-20 nodes per wavelength).

\vskip8mm

\noindent As said in section~\ref{sec:disper}, group velocity can be used to further describe wave packets and energy transport. Knowing the modified pulsation, the group velocity of the numerical scheme can be calculated the same way as the real one:
\begin{equation}
\label{eq:vgroup_star}
\mathcal{V}^{\star}_g (\xi) = \frac{d \omega^{\star}}{d\xi} = a \frac{d\xi^{\star}}{d\xi}.
\end{equation}

\noindent Depending on the resolution power of the difference schemes, some waves can have an unexpected behaviour. Group velocity is the main tool to discriminate those waves, as it gives information about the way they propagate. The bigger wavelenghts ($\xi h \rightarrow 0$) behaves like physical waves (consistency condition) and moves in the right direction, at the right velocity and dispersion errors are relatively small, especially when a high-order scheme is used. On the other hand, the smaller wavelengths ($\xi h \rightarrow \pi$), which are badly-resolved (only few points), are moving at a wrong speed, in the opposite direction. They can be considered as spurious waves, as they do not follow the physical laws and can threat the computation stability. Several illustrations are provided in \cite{Trefethen:1982,Vichnevetsky:1982}. Vichnevetsky and Bowles \cite{Vichnevetsky:1982} have called the first oscillations {\it p-waves} and the spurious ones {\it q-waves}.
...
\noindent {\it q-waves} can be very dangerous, as they carry some energy in the wrong direction or be stationnary, can reflect on boundaries and eventually lead to numerical instabilities \cite{Trefethen:1982,Vichnevetsky:1982}.
...
\end{equation}
\noindent and Figure~(\ref{fig:vg_lw}) displays it for different CFL numbers. As explained above, the scheme behaves rather well for $\xi h \rightarrow 0$, especially for higher CFL numbers. In general, phase errors are very sensible when $\lambda \approx 10-20 h$. When oscillations are smaller than around $4 h$, they are even convected in the wrong direction (especially when the CFL number is small). One should also note that there is no {\it q-waves} at higher CFL numbers (see $c=0.9$).
 \end{example}
...
So far, we have just considered phase errors, but most numerical schemes also introduce diffusion errors, as soon as $\Sigma^{\star}$ is not zero. Indeed, we have:
\begin{equation}
